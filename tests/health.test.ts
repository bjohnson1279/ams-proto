...
describe('Health and System Routes', () => {
  it('GET /health should return status healthy and uptime', async () => {
    const res = await request(app).get('/health').set('x-tenant-id', 'tenant-001');
  ...
  it('GET / should return root summary info', async () => {
    const res = await request(app).get('/').set('x-tenant-id', 'tenant-001').set('Accept', 'application/json');
    expect(res.status).toBe(200);
