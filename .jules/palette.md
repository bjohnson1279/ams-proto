...
## 2026-09-26 - Robust Search Input & Announcing Positive Results
**Learning:** For real-time search inputs, `onkeyup` fails to catch right-click pasting, drag-and-drop operations, and autofill. Furthermore, while empty state announcements (like "No results") are common, failing to announce positive search results (e.g., "Found 3 customers") leaves screen reader users guessing if the search succeeded or if the UI failed to update. Additionally, visual shortcut hints (like `<kbd>/`</kbd>) inside the input can clutter the user's text; dynamically fading them out (`opacity: 0`) when text is present provides a cleaner experience.
**Action:** Always use `oninput` for real-time text inputs to capture all mutation events. Ensure that both empty and non-empty data states trigger a polite screen reader announcement. Finally, dynamically hide visual shortcut hints within inputs when the user begins typing.
