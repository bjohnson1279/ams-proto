...
async function runImportTest() {
  ...
  const resA = await service.importLegacyPayload('tenant-001', formatAPayload as unknown as IngestionPayload);
  ...
  const resB = await service.importLegacyPayload('tenant-001', formatBPayload as unknown as IngestionPayload);
  ...
  const resC = await service.importLegacyPayload('tenant-001', formatCPayload as unknown as IngestionPayload);
  ...
  const resD = await service.importLegacyPayload('tenant-001', formatDPayload as unknown as IngestionPayload);
  ...
  const dryRunRes = await service.dryRunImport('tenant-001', formatDPayload as unknown as IngestionPayload);
  ...
  const allCustomers = await service.getCustomers('tenant-001');
  ...
  const decPage = await service.generateDecPage('tenant-001', 'POL-FMT-A-90214');
