...
// ⚡ Bolt: Moved static value lists outside the function to prevent object reallocation on every request
const VALUE_LISTS: Record<string, Array<{ code: string; description: string }>> = {
  EmployeeStatus: [
...
function getValueList(listName: string): Array<{ code: string; description: string }> | null {
  return VALUE_LISTS[listName] ?? null;
}
