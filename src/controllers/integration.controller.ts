...
export class IntegrationController {
  ...
  public importLegacyPayload = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const crosswalkResult = await this.amsService.importLegacyPayload((req as any).tenantId as string,  payload);
  ...
  public dryRunImport = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const crosswalkResult = await this.amsService.dryRunImport((req as any).tenantId as string,  payload);
      const hasCriticalExceptions = crosswalkResult.exceptions.some(e => e.severity === 'CRITICAL');
  ...
  public getCrosswalkMatrix = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const matrix = await this.amsService.getCrosswalkMatrix((req as any).tenantId as string);
      res.status(200).json({
        status: 'success',
        count: matrix.length,
        matrix,
